...
class BuDingController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    var BuDing:[[String]]=[]
    ...
    var parameters:[String : String] = ["FILE_SORT":"1","SORT_ID":"37","currentPage":"0","itemsPerPage":"10"]
        ...
        Alamofire.request(.POST,urll,parameters:parameters,headers: header ).responseData{ response in
            if let j = response.result.value{
                ...
                for var i=0;i<self.a;i+=1{
                    ...
                    if self.BuDing.count >= 10{
                        self.BuDing.removeLast()
                    }
                    self.BuDing.insert(xin[i],atIndex:0)
                }
...
//                })
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.titleView.text = self.BuDing[indexPath.row][1]
        cell.GroupEdit.text = self.BuDing[indexPath.row][2]
//        cell.GroupTime.text = self.BuDing[indexPath.row][3]
        return cell
    }
